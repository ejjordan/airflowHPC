...
from airflowHPC.dags.tasks import get_file, run_gmxapi_dataclass, prepare_gmxapi_input, unpack_ref_t
...
def unpack_gro_inputs(**context):
    ...
    """
    temps_range = range(len(context["task"].render_template("{{ params.ref_t_list | list}}", context)))
    gro_input_dir = context["task"].render_template("{{ params.inputs.gro.directory }}", context)
    num_steps = context["task"].render_template("{{ params.step_number }}", context)
    input_dir = [
        f"{gro_input_dir}/step_{num_steps}/sim_{i}" for i in temps_range
    ]
    return input_dir
...
def unpack_cpt_inputs(**context):
    ...
    """
    temps_range = range(len(context["task"].render_template("{{ params.ref_t_list | list}}", context)))
    cpt_input_dir = context["task"].render_template("{{ params.inputs.cpt.directory }}", context)
    num_steps = context["task"].render_template("{{ params.step_number }}", context)
    input_dir = [
        f"{cpt_input_dir}/step_{num_steps}/sim_{i}" for i in temps_range
    ]
    return input_dir
...
with DAG(
    ...
    params={
        "inputs": Param(
        {
            ...
            "gro": {"directory": "npt_equil", "filename": "result.gro"},
            "cpt": {"directory": "npt_equil", "filename": "result.cpt"},
    ...
    #inputs = simulate.params.get_param('inputs').value
    #ref_temps = simulate.params.get_param('ref_t_list').value
    #step_num = simulate.params.get_param('step_number').value
    #num_sims = len(ref_temps)
    ...
    mdp_sim = update_write_mdp_json_as_mdp_from_file.override(task_id="write_mdp_sim").partial(
        mdp_json_file_path=mdp_json_sim
    ).expand(
        update_dict=ref_temps
    )
    grompp_input_list_sim = prepare_gmxapi_input(
        ...
        output_dir=simulate.params.get_param('output_dir').value,
